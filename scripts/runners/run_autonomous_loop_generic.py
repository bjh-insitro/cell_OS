...
def run_autonomous_loop(candidates_json_path, workers=8):
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════╗
║          AUTONOMOUS LOOP EXPERIMENT FROM JSON                ║
╚══════════════════════════════════════════════════════════════╝
...
""")
    ...
    def custom_generate(candidates_list, mode):
        ...
        # 2 plates × (12 DMSO + 2 tBHQ + 2 tunicamycin) = 32 control wells

        # Get unique cell_line × timepoint combinations from candidates
        from collections import defaultdict
        combinations = set((c['cell_line'], c['timepoint_h']) for c in candidates_list)
        combinations_list = sorted(list(combinations))

        # Distribute 24 DMSO across combinations (12 per plate, ~6 per combination for 2 plates × 2 combinations)
        total_dmso = 24
        ...
        for combo_idx, (cell_line, timepoint) in enumerate(combinations_list):
            ...
            tbhq_dose = 1.0 * tbhq_params['ec50_uM']  # Use 1× EC50
        ...
        for combo_idx, (cell_line, timepoint) in enumerate(combinations_list):
            ...
            tunicamycin_dose = 1.0 * tunicamycin_params['ec50_uM']  # Use 1× EC50
...
""")
        ...
        try:
            ...
            try:
                ...
                print(f"   📥 Downloaded existing S3 database for merging...")
                ...
                print(f"   🔗 Merged new results into existing database")
                ...
                print(f"   📝 No existing S3 database, creating new one...")
            ...
            print(f"\n💡 Running locally (boto3 not available)")
