...
def _get_cell_line_baseline() -> dict:
    """
    Get baseline measurements for different cell lines (untreated).
    
    Cell line-specific nuclear sizes based on literature:
    • U2OS nuclei: often 120 to 200 µm²
    • A549 nuclei: around 90 to 150 µm²
    • HepG2 nuclei: tighter, maybe 80 to 130 µm²
    • iPSCs: smaller but denser, roughly 60 to 110 µm²
    
    Returns dict with cell line-specific baselines.
    """
    return {
        "U2OS": {
            # Mitochondria
            "mito_mean_intensity": (35000, 4000),
            "mito_object_count": (20, 5),
            "mito_total_area": (10000, 2000),
            "mito_texture_variance": (800, 200),
            # Nucleus
            "nucleus_area": (160, 25),  # 120-200 µm² range
            "nucleus_perimeter": (52, 8),
            "nucleus_mean_intensity": (28000, 3000),
            "nucleus_form_factor": (0.85, 0.05),
            "nucleus_eccentricity": (0.25, 0.08),
        },
        "A549": {
            # Mitochondria
            "mito_mean_intensity": (33000, 4000),
            "mito_object_count": (22, 5),
            "mito_total_area": (9500, 2000),
            "mito_texture_variance": (850, 200),
            # Nucleus
            "nucleus_area": (120, 20),  # 90-150 µm² range
            "nucleus_perimeter": (45, 7),
            "nucleus_mean_intensity": (27000, 3000),
            "nucleus_form_factor": (0.83, 0.06),
            "nucleus_eccentricity": (0.28, 0.08),
        },
        "HepG2": {
            # Mitochondria
            "mito_mean_intensity": (36000, 3500),
            "mito_object_count": (18, 4),
            "mito_total_area": (11000, 2000),
            "mito_texture_variance": (750, 180),
            # Nucleus
            "nucleus_area": (105, 18),  # 80-130 µm² range
            "nucleus_perimeter": (42, 6),
            "nucleus_mean_intensity": (29000, 3000),
            "nucleus_form_factor": (0.87, 0.04),
            "nucleus_eccentricity": (0.22, 0.07),
        },
        "iPSC": {
            # Mitochondria
            "mito_mean_intensity": (32000, 4000),
            "mito_object_count": (25, 6),
            "mito_total_area": (8500, 1800),
            "mito_texture_variance": (900, 220),
            # Nucleus
            "nucleus_area": (85, 15),  # 60-110 µm² range
            "nucleus_perimeter": (38, 6),
            "nucleus_mean_intensity": (30000, 3500),  # Denser chromatin
            "nucleus_form_factor": (0.88, 0.04),
            "nucleus_eccentricity": (0.20, 0.06),
        }
    }


def _get_treatment_raw_measurement_effects(treatment: str, cell_line: str = "U2OS") -> dict:
    """
    Get baseline effects of treatment on raw imaging measurements.
    
    Starts with cell line-specific baseline, then applies treatment-specific modifiers.
    
    Args:
        treatment: Treatment name (e.g., "tBHP")
        cell_line: Cell line name (e.g., "U2OS")
    
    Returns dict with mean and std for each raw measurement.
    """
    # Get cell line baseline
    cell_line_baselines = _get_cell_line_baseline()
    baseline = cell_line_baselines.get(cell_line, cell_line_baselines["U2OS"])
    
    # Apply treatment-specific modifications
    if treatment == "tBHP":  # Oxidative stress
        return {
            # Mitochondria: moderate damage
            "mito_mean_intensity": (baseline["mito_mean_intensity"][0] * 0.7, 5000),
            "mito_object_count": (baseline["mito_object_count"][0] * 2.2, 10),
            "mito_total_area": (baseline["mito_total_area"][0] * 0.8, 1500),
            "mito_texture_variance": (baseline["mito_texture_variance"][0] * 1.5, 300),
            # Nucleus: slight shrinkage
            "nucleus_area": (baseline["nucleus_area"][0] * 0.9, baseline["nucleus_area"][1]),
            "nucleus_perimeter": (baseline["nucleus_perimeter"][0] * 0.95, baseline["nucleus_perimeter"][1]),
            "nucleus_mean_intensity": (baseline["nucleus_mean_intensity"][0] * 1.15, 4000),
            "nucleus_form_factor": (0.75, 0.1),
            "nucleus_eccentricity": (0.4, 0.1),
        }
    elif treatment == "Staurosporine":  # Apoptosis
        return {
            # Mitochondria: severe damage
            "mito_mean_intensity": (baseline["mito_mean_intensity"][0] * 0.5, 4000),
            "mito_object_count": (baseline["mito_object_count"][0] * 2.0, 8),
            "mito_total_area": (baseline["mito_total_area"][0] * 0.6, 1200),
            "mito_texture_variance": (baseline["mito_texture_variance"][0] * 1.2, 250),
            # Nucleus: strong shrinkage (apoptosis)
            "nucleus_area": (baseline["nucleus_area"][0] * 0.7, baseline["nucleus_area"][1] * 0.8),
            "nucleus_perimeter": (baseline["nucleus_perimeter"][0] * 0.8, baseline["nucleus_perimeter"][1]),
            "nucleus_mean_intensity": (baseline["nucleus_mean_intensity"][0] * 1.6, 5000),
            "nucleus_form_factor": (0.55, 0.12),
            "nucleus_eccentricity": (0.65, 0.12),
        }
    elif treatment == "Tunicamycin":  # ER stress
        return {
            # Mitochondria: mild effect
            "mito_mean_intensity": (baseline["mito_mean_intensity"][0] * 0.85, 5000),
            "mito_object_count": (baseline["mito_object_count"][0] * 1.7, 8),
            "mito_total_area": (baseline["mito_total_area"][0] * 0.75, 1500),
            "mito_texture_variance": (baseline["mito_texture_variance"][0] * 1.6, 300),
            # Nucleus: slight swelling (stress)
            "nucleus_area": (baseline["nucleus_area"][0] * 1.05, baseline["nucleus_area"][1]),
            "nucleus_perimeter": (baseline["nucleus_perimeter"][0] * 1.03, baseline["nucleus_perimeter"][1]),
            "nucleus_mean_intensity": (baseline["nucleus_mean_intensity"][0] * 1.07, 4000),
            "nucleus_form_factor": (0.78, 0.08),
            "nucleus_eccentricity": (0.35, 0.08),
        }
    else:
        # No treatment - return baseline
        return baseline
