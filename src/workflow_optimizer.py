...
class WorkflowOptimizer:
    ...
    def _normalize_operations(self, operations: Any) -> List[UnitOp]:
        """
        Accept either:
          - a List[UnitOp]
        ...
        # Assume it's already an iterable of UnitOp
    ...
    def analyze_workflow(
        ...
        """
        ops_list = self._normalize_operations(operations)
    ...
    def calculate_roi(
        ...
        payback_months = (
            switching_cost / monthly_savings if monthly_savings > 0 else float("inf")
        )
    ...
    def generate_optimization_report(
        ...
        # Normalize to a flat op list for downstream logic
        ops_list = self._normalize_operations(operations)

        # Find passage and freeze operations
        passage_ops = [
            op for op in ops_list
            if "Passage" in op.name or "passage" in op.uo_id.lower()
        ]
        freeze_ops = [
            op for op in ops_list
            if "Freeze" in op.name or "freeze" in op.uo_id.lower()
        ]
        ...
        if passage_ops:
            ...
            alternatives = self.suggest_passage_alternatives(
                current_method, "plate_6well", cell_type
            )
            ...
            for alt in alternatives[:3]:  # Top 3
                ...
                report.append(
                    f"  Savings: ${alt.savings_usd:.2f} per run ({alt.savings_pct:.1f}%)"
                )
        ...
        if freeze_ops:
            ...
            alternatives = self.suggest_freezing_alternatives(
                current_media, 10, cell_type
            )
        ...
        report.append("\n=== Summary ===")
        report.append(
            f"Total potential annual savings: ${total_potential_savings:.2f}"
        )
        if analysis["monthly_cost"] > 0:
            report.append(
                "Savings as % of current annual cost: "
                f"{(total_potential_savings / (analysis['monthly_cost'] * 12) * 100):.1f}%"
            )
        else:
            report.append("Savings as % of current annual cost: N/A")
