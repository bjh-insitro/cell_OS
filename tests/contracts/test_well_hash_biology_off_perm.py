"""
Permutation test for Biology OFF condition.
"""
...
def test_biology_off_permutation():
    """Permutation test for Biology OFF."""
    print("=" * 70)
    print("BIOLOGY OFF PERMUTATION TEST")
    print("=" * 70)

    measurements = generate_dmso_plate_biology_off(seed=42)
    auc_real = compute_auc(measurements)

    print(f"\nReal AUC (Biology OFF): {auc_real:.4f}")
    print("\nRunning 1000 permutations...")

    null_aucs = []
    for i in range(1000):
        auc = compute_auc(measurements, shuffle_seed=2000 + i)
    ...
    if p_value < 0.05:
        print("\n⚠️  SIGNIFICANT: Biology OFF shows edge leakability")
        print("   Residual leak from other sources (not well_biology)")
    else:
        print("\n✅ NOT SIGNIFICANT: Biology OFF edge signal within chance")


if __name__ == "__main__":
    test_biology_off_permutation()
