"""
Attack 2.2 Cut 2: Null hardware plating bias.

Test if get_hardware_bias(well_position) is the 0.77 leak.
"""
...
# Monkeypatch get_hardware_bias to return identity
def null_hardware_bias(*args, **kwargs):
    return {'volume_factor': 1.0, 'roughness_factor': 1.0}


def generate_dmso_plate_null_hardware_bias(seed: int = 42):
    """
    Generate DMSO plate with well biology OFF, edge effects OFF, hardware bias OFF.
    """
    # Monkeypatch before importing VM
...
def test_cut2():
    """Test edge AUC with hardware plating bias nulled."""
    from sklearn.linear_model import LogisticRegression
    from sklearn.metrics import roc_auc_score
    from sklearn.preprocessing import StandardScaler

    print("=" * 70)
    print("ATTACK 2.2 CUT 2: Null Hardware Plating Bias")
    print("=" * 70)

    measurements = generate_dmso_plate_null_hardware_bias(seed=42)
    ...
    print(f"\nEdge AUC (hardware bias nulled): {auc:.4f}")
    print(f"Baseline (both OFF from Attack 2.1): 0.7656")
    print(f"Delta: {auc - 0.7656:.4f}")

    if auc < 0.65:
        print("\n✅ LEAK ISOLATED: Hardware plating bias was the 0.77 source")
    else:
