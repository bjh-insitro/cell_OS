"""
Attack 2.2 Cut 1: Null technical noise well factors.

Test if well_factor_{well_position}_{channel} is the 0.77 leak.
"""
...
def generate_dmso_plate_null_well_factors(seed: int = 42):
    """
    Generate DMSO plate with well biology OFF, edge effects OFF, well factors OFF.
    """
    # Disable edge effects via realism config
    realism_config = {
    ...
    # NULL WELL FACTORS: set well_cv to 0
    vm.thalamus_params['technical_noise']['well_cv'] = 0.0
...
def test_cut1():
    """Test edge AUC with technical well factors nulled."""
    from sklearn.linear_model import LogisticRegression
    from sklearn.metrics import roc_auc_score
    from sklearn.preprocessing import StandardScaler

    print("=" * 70)
    print("ATTACK 2.2 CUT 1: Null Technical Well Factors")
    print("=" * 70)

    measurements = generate_dmso_plate_null_well_factors(seed=42)
    ...
    print(f"\nEdge AUC (well factors nulled): {auc:.4f}")
    print(f"Baseline (both OFF from Attack 2.1): 0.7656")
    print(f"Delta: {auc - 0.7656:.4f}")

    if auc < 0.65:
        print("\n✅ LEAK ISOLATED: Technical well factors were the 0.77 source")
    else:
        print(f"\n⚠️  Leak persists (AUC={auc:.4f}), check next toggle")
