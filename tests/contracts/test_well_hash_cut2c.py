"""
Attack 2.2 Cut 2c: Null edge_effect in technical_noise.

Test if tech_noise['edge_effect'] is the 0.77 leak.
"""
...
def generate_dmso_plate_null_tech_edge(seed: int = 42):
    """
    Generate DMSO plate with well biology OFF, all edge effects OFF.
    """
    ...
    # NULL technical noise edge_effect
    vm.thalamus_params['technical_noise']['edge_effect'] = 0.0
...
def test_cut2c():
    """Test edge AUC with technical_noise edge_effect nulled."""
    from sklearn.linear_model import LogisticRegression
    from sklearn.metrics import roc_auc_score
    from sklearn.preprocessing import StandardScaler

    print("=" * 70)
    print("ATTACK 2.2 CUT 2c: Null Technical Noise Edge Effect")
    print("=" * 70)

    measurements = generate_dmso_plate_null_tech_edge(seed=42)
    ...
    print(f"\nEdge AUC (tech edge_effect nulled): {auc:.4f}")
    print(f"Baseline (both OFF from Attack 2.1): 0.7656")
    print(f"Delta: {auc - 0.7656:.4f}")

    if auc < 0.65:
        print("\n✅ LEAK ISOLATED: Technical noise edge_effect was the 0.77 source")
    else:
        print(f"\n⚠️  Leak persists (AUC={auc:.4f})")
        print("Remaining candidates: batch factors, stain, evaporation, or stochastic RNG seeding")
