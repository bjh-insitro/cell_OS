...
def test_evaporation_drift_affects_attrition():
    ...
    - Same initial dose (moderate to get attrition)
    - Advance time without ops
    - Assert edge has more death from concentrated exposure

    Use death_compound accumulation to isolate attrition effect.
    """
    ...
    # Same dose at time 0 (moderate dose to get measurable attrition)
    ...
    # Advance 72 hours (longer time for attrition to accumulate)
    print(f"\nAdvancing 72 hours...")
    ...
    print(f"\nFinal state (after 72h attrition):")
    ...
    # Allow for small numerical drift - require at least 1% difference
    if attrition_edge > attrition_int * 1.01:
        print(f"✓ PASS: Edge well has more attrition (higher concentrated exposure)")
        return True
    else:
        print(f"❌ FAIL: Edge well does not have meaningfully more attrition (edge={attrition_edge:.4f}, int={attrition_int:.4f})")
