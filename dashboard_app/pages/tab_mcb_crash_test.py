...
st.set_page_config(
    page_title="MCB Crash Test Analysis",
    page_icon="🧬",
    layout="wide"
)

# Constants
ASSETS_DIR = Path("data/dashboard_assets/mcb")
...
def load_assets():
    ...
    # Load Summary JSON
    try:
        with open(ASSETS_DIR / "mcb_summary.json", "r") as f:
            assets["summary"] = json.load(f)
    except FileNotFoundError:
        st.warning("mcb_summary.json not found.")
        
    # Load Run Results CSV
    try:
        assets["run_results"] = pd.read_csv(ASSETS_DIR / "mcb_run_results.csv")
    except FileNotFoundError:
        st.warning("mcb_run_results.csv not found.")
        
    # Load Daily Metrics CSV
    try:
        assets["daily_metrics"] = pd.read_csv(ASSETS_DIR / "mcb_daily_metrics.csv")
    except FileNotFoundError:
        st.warning("mcb_daily_metrics.csv not found.")
        
    # Load Plots Manifest (for static plots if needed)
    try:
        with open(ASSETS_DIR / "plots_manifest.json", "r") as f:
            assets["plots"] = json.load(f)
    except FileNotFoundError:
        pass
...
def render_metrics(summary):
    ...
    with col3:
        st.metric(
            ...
            f"{summary.get('waste_fraction_p50', 0):.1%}",
            f"{summary.get('waste_p50', 0):.0f} vials"
        )
...
def render_vial_distribution(df):
    ...
    chart = alt.Chart(df).mark_bar().encode(
        x=alt.X("final_vials:Q", bin=alt.Bin(maxbins=30), title="Final Vials Generated"),
...
def render_waste_analysis(df):
    ...
    with col1:
        # Waste Fraction Histogram
        chart = alt.Chart(df).mark_bar().encode(
            x=alt.X("waste_fraction:Q", bin=alt.Bin(maxbins=20), title="Waste Fraction"),
            y=alt.Y("count()", title="Count"),
            tooltip=["count()", "waste_fraction"]
        ).properties(
    ...
    with col2:
        ...
        chart = alt.Chart(df).mark_circle(size=60).encode(
            x=alt.X("final_vials:Q", title="Final Vials Banked"),
            y=alt.Y("waste_vials:Q", title="Waste Vials Discarded"),
            color=alt.Color("terminal_failure:N", title="Failed"),
            tooltip=["run_id", "final_vials", "waste_vials", "waste_fraction"]
        ).properties(
...
def main():
    ...
    st.markdown("Analysis of Monte Carlo simulations for Master Cell Bank production.")
    
    assets = load_assets()
    if not assets:
        st.info("Run the simulation first to generate assets.")
        return
        
    if "summary" in assets:
        render_metrics(assets["summary"])
        
    tab1, tab2, tab3, tab4 = st.tabs(["Production & Waste", "Growth Dynamics", "Failures", "Raw Data"])
    
    with tab1:
        if "run_results" in assets:
            render_vial_distribution(assets["run_results"])
            render_waste_analysis(assets["run_results"])
